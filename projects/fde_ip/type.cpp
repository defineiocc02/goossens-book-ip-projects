/**
 * @file type.cpp
 * @brief RISC-V 指令类型识别模块
 * 
 * 本模块根据 opcode 识别 RISC-V 指令的类型
 * RISC-V 指令格式：
 * - R-type: 寄存器-寄存器操作（如 ADD, SUB, AND, OR 等）
 * - I-type: 立即数操作（如 ADDI, LW, JALR 等）
 * - S-type: 存储操作（如 SW, SH, SB 等）
 * - B-type: 分支操作（如 BEQ, BNE, BLT 等）
 * - U-type: 长立即数（如 LUI, AUIPC）
 * - J-type: 跳转操作（如 JAL）
 */
...
/**
 * @brief 处理 opcode 高2位为 00 的指令类型
 * @param opcl opcode 低3位
 * @return 指令类型
 */
